...
const [data,setData] = useState([])
  useEffect(() => {
    async function getProducts() {
        const res = await fetch("/api/products?subcategory=chair&limit=5&sort=newest",{cache:"no-store"})
        if(!res.ok) {
...
const data =  await res.json()
  ...
  <Link href="/products/living-room/chair" className='underline text-white p-2'>
  View all
  </Link>
      </div>
      </div>
